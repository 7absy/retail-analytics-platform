...
# MAGIC %md # Bronze — product_catalog
...
# COMMAND ----------
RAW_PATH    = f"abfss://raw@{account_name}.dfs.core.windows.net/product_catalog"
BRONZE_PATH = f"abfss://curated@{account_name}.dfs.core.windows.net/bronze/product_catalog"
...
if DeltaTable.isDeltaTable(spark, BRONZE_PATH):
    DeltaTable.forPath(spark, BRONZE_PATH).alias("target").merge(
        df.alias("source"), "target.product_id = source.product_id"
...
print(f"bronze_product_catalog row count: {count}")
