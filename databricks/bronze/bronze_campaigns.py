...
# MAGIC %md # Bronze — campaigns (API JSON)
...
RAW_PATH    = f"abfss://raw@{account_name}.dfs.core.windows.net/api/campaigns/{DATE_STR}/campaigns.json"
BRONZE_PATH = f"abfss://curated@{account_name}.dfs.core.windows.net/bronze/campaigns"
...
if DeltaTable.isDeltaTable(spark, BRONZE_PATH):
    DeltaTable.forPath(spark, BRONZE_PATH).alias("target").merge(
        df.alias("source"), "target.campaign_id = source.campaign_id"
...
print(f"bronze_campaigns row count: {count}")
